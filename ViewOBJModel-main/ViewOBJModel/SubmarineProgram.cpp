...
void SubmarineProgram::RenderObjects(Shader* shader) {
	...
	waterMatrix = glm::scale(waterMatrix, glm::vec3(10.0f));
	waterMatrix = glm::rotate(waterMatrix, glm::radians(90.f), glm::vec3(1.f, 0.f, 0.f));
	waterMatrix = glm::translate(waterMatrix, glm::vec3(0.0f, -15.0f, 0.0f));
	...
	waterMatrix = glm::translate(glm::mat4(1.f), camera->GetPosition());

	forward = camera->GetRealForward();
	rotationMatrix = glm::inverse(glm::lookAt(glm::vec3(0.0f), forward, up));

	waterMatrix *= rotationMatrix;
	//waterMatrix = glm::scale(waterMatrix, glm::vec3(10.f));
	waterMatrix = glm::translate(waterMatrix, glm::vec3(0.0f, 0.0f, -5.0f));

	float depth = min(0.7f, max(0.f, ((-camera->GetPosition().y - 25.f) / 50.f)));

	shader->setMat4("model", waterMatrix);
	lightingWithTextureShader->setFloat("opacity", depth);
